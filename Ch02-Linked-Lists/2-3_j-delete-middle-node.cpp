 ...
 */
void deleteNode( Node * current_node_want_del ) {
  if ( current_node_want_del == nullptr || current_node_want_del->next == nullptr ) {
    return;
  }
  Node * nextNode = current_node_want_del->next;
  current_node_want_del->data = nextNode->data;
  current_node_want_del->next = nextNode->next;
  //replaced the current node value by next,next value,
  //which means we drop the current data
  delete nextNode;
}
