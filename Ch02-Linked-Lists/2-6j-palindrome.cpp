...
bool isPalindromeIter1( Node * head ) {
  ...
  while( ptr2 && ptr1 && ptr1->next) {
    ptr1 = ptr1->next->next;//move two node
    ptr2 = ptr2->next;//move one node
  }
  ...
  while( ptr1 && ptr2 && ptr1->data == ptr2->data ) {
    ...
    std::cout << "ptr1 -> next address" << ptr1 ->next << std::endl;
    std::cout << "ptr2 address" << ptr2  << std::endl;
    std::cout << "ptr2 -> next address" << ptr2 ->next << std::endl;
  ...
  std::cout << "middleNode address, before reverse " << middleNode << std::endl;
  reverse(middleNode);
  std::cout << "middleNode address " << middleNode << std::endl;
  //ptr1 -> next was always point to the original node order.
  //so we just reerse the bottom half list is enough.
...
bool isPalindromeRecurHelper( Node * & left, Node * right ) {
  ...
  //keep moving right point to the end of list
  bool isPalindrome = isPalindromeRecurHelper(left, right->next);
  if (!isPalindrome) {
    return false;
  }

  // we start comparing the data in left-right pairs.
...
int main()
{
  ...
  //stack ver to replace the reverse fun
  Node * head2 = nullptr;
  insert( head2, 'r');
  insert( head2, 'a');
  insert( head2, 'd');
